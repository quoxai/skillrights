...
export function runVerify(positional, flags) {
  ...
  try {
    ...
    return { ok: false, findings: [`Manifest is not valid JSON: ${err.message}`], dir };
  }
  ...
  if (signersPath) {
    if (!fileExists(sigPath)) {
      ...
      if (!principal) {
        ...
        if (result.status === 0) {
          findings.push(`PASS: signature verified for principal "${principal}".`);
        } else {
          ok = false;
          findings.push(
    ...
    findings.push(`NOTE: a signature file exists at ${sigPath} but no --signers file was given; signature was not checked.`);
  }

  return { ok, findings, dir };
}
