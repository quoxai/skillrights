...
That is the whole declaration; about 20 tokens, loaded only when the skill is invoked. Distributing, interpreting and enforcing it are their own integrations (a public conformance suite covers the parsing and decision layer). No AI model needs to read legal prose for the declaration to work.
...
| `SkillRights-NoTrain-1.0` | yes | **reserved** | yes (attribution; modified copies stay under this licence) | yes |
| `SkillRights-Reserved-1.0` | authorised recipients, internal business use only | **reserved** | **reserved** | by agreement |
...
`skillrights register` (CLI 0.2.4) adds the independent evidence: it registers the signed hash in the SkillRights Registry, an append-only transparency log at registry.skillrights.org that returns a permanent SRID and a portable receipt (Merkle inclusion proof plus an Ed25519-signed tree head), verified locally before it is saved. Registration is free, needs no account, and in unlisted mode sends only the hash, signature, licence identifier and claimed author: the skill's content never leaves your machine. Unlisted is not confidential: the log is public in both modes, and the mode controls only the directory listing at skillrights.org/registry. A registration establishes that the artefact existed by the recorded time, and records the key and signature submitted with it, marked verified only where the registry could check the signature against the registered hash. The CLI signs that hash with your existing Ed25519 SSH key so the check can be made; where the key is passphrase-protected, missing or not ed25519 it falls back to the manifest-file signature, which the registry cannot check, and says so. Registration does not prove legal ownership, authorship or originality.

The log is never its own only witness. The anchoring worker requests an independent RFC 3161 countersignature for each new tree head and submits it to public OpenTimestamps calendar servers, which batch it into the Bitcoin blockchain (proofs upgrade in place once the calendars commit, typically within hours; per-root anchor state, including pending and failed attempts, is public at `GET /api/v1/log/anchors`). Anyone can mirror the full log (`GET /api/v1/log/entries`), recompute the Merkle root, download the anchor proofs (`GET /api/v1/log/anchors`) and check them with standard tooling (`ots verify`, `openssl ts -verify`), with no trust in skillrights.org required. The full recipe is in [registry/MIRRORING.md](../registry/MIRRORING.md).
